// src/services/imageGenerator.ts - Servicio de generación de imágenes
...
import { wrapText, drawRoundedRect, drawGlassmorphismContainer } from '../utils/canvas'
...
const logger = winston.createLogger({
  ...
  transports: [
    new winston.transports.File({ filename: 'logs/image-generator.log' })
  ]
})
...
// Inicializar fuentes al importar el módulo
initializeFonts()

type ImageFormat = 'png' | 'jpg' | 'jpeg' | 'webp';

// Cargar los iconos SVG una sola vez (al iniciar el módulo)
let commentIcon: any, retweetIcon: any, heartIcon: any, statsIcon: any
...
async function loadIcons() {
  if (!iconsLoaded) {
    [commentIcon, retweetIcon, heartIcon, statsIcon] = await Promise.all([
      ...
      loadImage('src/icons/stats.svg')
    ])
    iconsLoaded = true
  }
}

async function extractFrameFromVideo(videoPath: string, width: number, height: number, requestId: string): Promise<string> {
  ...
  try {
    ...
    // Extrae el primer frame en la resolución deseada
    ...
    execSync(command, {
      stdio: 'pipe' // Silenciar output de FFmpeg
    })
...
export async function generateSocialImage(
  tweetData: TweetData,
  options: ImageOptions & { backgroundImage?: string; aspectRatio?: string },
  requestId: string
): Promise<GenerationResponse> {
  const startTime = Date.now()

  // --- NUEVO: Soporte para aspectRatio tipo '4:5' ---
  ...
  try {
    ...
    // Configurar glassmorphism según el tema
    let glassConfig = getGlassmorphismConfig(tweetData.theme || options.theme || 'dark')
    // Forzar texto claro si fondo azul o gradiente y overlay desactivado
    ...
    // Dibujar fondo con las nuevas opciones
    await drawBackground(
      ctx, 
      canvas.width, 
      canvas.height, 
      options.backgroundType || 'blue',
      options.backgroundVideo,
      requestId
    )

    // Dibujar contenido del tweet con overlay opcional
...
function getGlassmorphismConfig(theme: 'light' | 'dark'): GlassmorphismConfig {
  if (theme === 'light') {
    return {
    ...
    case 'video-frame':
      if (backgroundVideo && requestId) {
        try {
          ...
          // Si es video, extrae el primer frame
          ...
          // Escalar para cubrir toda la imagen manteniendo proporción
...
// Nueva función con diseño unificado (igual que videos)
async function drawTweetContentWithOverlay(
  ctx: CanvasRenderingContext2D,
  data: TweetData,
  width: number,
  height: number,
  ...
  // Usar exactamente la misma lógica que drawTweetOverlay del canvas.ts
  await loadIcons()
  ...
  if (isOneOne) {
    overlayWidth = width * 0.9  // Incrementado de 0.85 a 0.9
  ...
  // Altura del overlay
  let overlayMaxHeight: number
  if (isOneOne) {
    overlayMaxHeight = height * 0.85  // Incrementado específicamente para 1:1
  ...
  if (height / width > 1.7 || isNineSixteen) fontBoost = 1.7
  if (isFourFive) fontBoost *= 1.1
  if (isThreeFour) fontBoost *= 1.05
  if (isOneOne) fontBoost *= 1.1  // Incrementado de 0.95 a 1.1 para que se vea más grande
  ...
  if (textLength > 200) baseFontSize *= 0.9
  else if (textLength > 100) baseFontSize *= 0.95
  else if (textLength < 50) baseFontSize *= 1.1
  ...
  // Ajustar fontSize hasta que quepa
  ctx.font = `${Math.round(tweetFontSize)}px TwitterChirp, Arial, sans-serif`
  let textLines = wrapText(ctx, data.text, textMaxWidth, lineHeight)
  ...
  for (let shrink = 0; shrink < 30; shrink++) {
    if (tweetTextHeight <= containerHeight - headerHeight - statsHeight - (paddingY * 2) || tweetFontSize < 14) break
    tweetFontSize *= 0.94
  ...
  try {
    ...
    if (avatarPath) {
      ...
      ctx.drawImage(
        ...
        avatarX - avatarSize / 2, avatarY - avatarSize / 2, avatarSize, avatarSize
      )
      ctx.restore()
    } else {
  ...
  const stats = []
  if (typeof data.replies === 'number') stats.push({ emoji: '💬', value: data.replies })
  if (typeof data.retweets === 'number') stats.push({ emoji: '🔄', value: data.retweets })
  if (typeof data.likes === 'number') stats.push({ emoji: '❤️', value: data.likes })
  if (typeof data.views === 'number') stats.push({ emoji: '👁️', value: data.views })
...
// Función para contenido directo (sin overlay)
async function drawTweetContentDirect(
  ctx: CanvasRenderingContext2D,
  data: TweetData,
  width: number,
  height: number,
  ...
  let fontBoost = 1
  if (isNineSixteen) fontBoost = 1.7
  else if (isFourFive) fontBoost = 1.3
  else if (isThreeFour) fontBoost = 1.15
  else if (isOneOne) fontBoost = 1.0
  else if (aspectRatio < 1) fontBoost = 1.2
  ...
  const textLines = wrapText(ctx, data.text, textMaxWidth, lineHeight)
  for (const line of textLines) {
...
async function drawTweetContent(
  ctx: CanvasRenderingContext2D,
  data: TweetData,
  width: number,
  height: number,
  ...
  const isThreeFour = Math.abs(aspectRatio - 0.75) < 0.05;
  const isOneOne = Math.abs(aspectRatio - 1) < 0.05;
  // OverlayWidth siempre 70% del ancho
  ...
  // Construir el array de stats para renderizar
  const stats = []
  if (typeof data.replies === 'number') stats.push({ icon: commentIcon, value: data.replies })
  if (typeof data.retweets === 'number') stats.push({ icon: retweetIcon, value: data.retweets })
  if (typeof data.likes === 'number') stats.push({ icon: heartIcon, value: data.likes })
}
...
// Función para generar imagen con plantilla personalizada
export async function generateImageWithTemplate(
  tweetData: TweetData,
  templateName: string,
  customOptions: Partial<ImageOptions> = {},
  requestId: string
): Promise<GenerationResponse> {
  ...
  const options: ImageOptions = { ...template, ...customOptions }
  
  return generateSocialImage(tweetData, options, requestId)
}

// Función para generar imagen con dimensiones personalizadas
export async function generateCustomSizeImage(
  tweetData: TweetData,
  width: number,
  height: number,
  requestId: string
...
// Función para generar múltiples formatos de una imagen
export async function generateMultipleFormats(
  tweetData: TweetData,
  baseOptions: ImageOptions,
  requestId: string
): Promise<Array<GenerationResponse & { format: string }>> {
  
  const formats: Array<ImageOptions['format']> = ['png', 'jpg', 'webp']
  const results: Array<GenerationResponse & { format: string }> = []
...
// Función para validar y optimizar imagen de avatar
export async function processAvatarImage(
  avatarPath: string,
  requestId: string
): Promise<{ optimizedPath: string; size: string }> {
  
  try {
    const avatarImage = await loadImage(avatarPath)
    
    // Crear canvas cuadrado para el avatar
    const size = 400
    const canvas = createCanvas(size, size)
    const ctx = canvas.getContext('2d')

    // Determinar cómo escalar y recortar la imagen
    const sourceSize = Math.min(avatarImage.width, avatarImage.height)
    const sourceX = (avatarImage.width - sourceSize) / 2
    const sourceY = (avatarImage.height - sourceSize) / 2
    ...
    // Guardar avatar optimizado
    await fs.mkdir('uploads/avatars', { recursive: true })
    const optimizedPath = path.join('uploads/avatars', `optimized-${requestId}.png`)
